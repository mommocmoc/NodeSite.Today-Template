...
export function GalleryGrid({
  ...
  const blocksToRender =
    collectionData?.blockIds || galleryBlocks.map((b) => b.value.id)
    ...
    <div className={styles.galleryGrid}>
      {blocksToRender.length > 0
        ? blocksToRender.map((blockId: string) => {
            ...
            const coverImage = getPageProperty<string>(
              'Cover',
              block,
              recordMap
            )
            ...
            const findImageProperty = () => {
              ...
              if (collection?.schema) {
                for (const [propId, propSchema] of Object.entries(
                  collection.schema
                )) {
                  const schema = propSchema as any
                  if (schema.type === 'file' || schema.type === 'media') {
                    const prop = getPageProperty<string>(
                      schema.name,
                      block,
                      recordMap
                    )
            ...
            if (
              ...
              block.format.page_icon.startsWith('http')
            ) {
